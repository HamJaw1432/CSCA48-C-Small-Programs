...
{
    // TODO in O(1) time
	Ticket *tmp_ticket_ptr = (Ticket *)calloc(1, sizeof(Ticket));
	if(tmp_ticket_ptr != NULL && data != NULL)
	{
		if(data -> last != NULL)
		{
			//printf("%p\n", tmp_ticket_ptr);
			tmp_ticket_ptr -> next = NULL;
			strcpy(tmp_ticket_ptr -> name, name);
			//printf("%p %p\n", tmp_ticket_ptr, data -> last -> name);
			data -> last -> next = tmp_ticket_ptr;
			//printf("%p\n", tmp_ticket_ptr);
			data -> last = tmp_ticket_ptr;
			//printf("%p\n", tmp_ticket_ptr);
		}
		else
		{
...
bool dequeue(Data *data, char *returned_name)
{
	...
	{
		Ticket *temp = data -> first;
		data -> first = data -> first -> next;
		free(temp);
		strcpy(returned_name, data -> first -> name);
		//printf("%s", returned_name);
...
bool push(Data *data, char *name)
{
    // TODO in O(1) time
	Ticket *tmp_ticket_ptr = (Ticket *)calloc(1, sizeof(Ticket));
	if(data != NULL && tmp_ticket_ptr != NULL)
	{
		tmp_ticket_ptr -> next = data -> first;
		strcpy(tmp_ticket_ptr -> name, name);

...
void wordSwapper(char *source, char *destination)
{
  ...
  while (destinationlen < len)
  {
	  while (wordstart >= 0 && *(source + wordstart)!= ' ')
	  {
		  wordstart = wordstart - 1;
	  } 
	  
	  for (int k = 1 ; k <= wordend - wordstart; k++) 
	  {
		  *(destination + destinationlen + k - 1) = *(source + wordstart + k);
	  }
	  destinationlen = destinationlen + wordend - wordstart;
	  *(destination + destinationlen - 1) = ' ';
	  wordend = wordstart;
	  wordstart = wordstart - 1;
  }
